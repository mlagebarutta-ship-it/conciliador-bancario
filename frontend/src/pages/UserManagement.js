...
export default function UserManagement() {
  ...
  const handleToggleStatus = async (user) => {
    ...
    try {
      await api.put(`/usuarios/${user.id}`, { status: newStatus });
  ...
  const handleLinkCompany = async (empresaId) => {
    try {
      await api.post(`/usuarios/${selectedUser.id}/empresas?empresa_id=${empresaId}`, {});
      toast.success('Empresa vinculada');
      
      // Recarregar dados do usuário
      const response = await api.get(`/usuarios/${selectedUser.id}`);
      setSelectedUser(response.data);
    } catch (error) {
  ...
  const handleUnlinkCompany = async (empresaId) => {
    try {
      await api.delete(`/usuarios/${selectedUser.id}/empresas/${empresaId}`);
