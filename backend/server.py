...
async def superadmin_dashboard(current_user: Dict = Depends(require_super_admin)):
    """Dashboard do Super Admin com métricas da plataforma"""
    ...
    # Top 5 escritórios por empresas
...
async def list_tenants(current_user: Dict = Depends(require_super_admin)):
    ...
    # Adicionar contagens para cada tenant
...
async def create_tenant(tenant_data: TenantCreate, current_user: Dict = Depends(require_super_admin)):
    ...
    await log_activity(
        ...
        "Escritório criado",
        detalhes=f"Nome: {tenant.nome}, CNPJ: {tenant.cnpj}"
    )
...
async def get_tenant(tenant_id: str, current_user: Dict = Depends(require_super_admin)):
    """Retorna detalhes de um escritório"""
    tenant = await db.tenants.find_one({"id": tenant_id}, {"_id": 0})
    if not tenant:
        raise HTTPException(status_code=404, detail="Escritório não encontrado")
    
    # Adicionar dados relacionados
    tenant["usuarios"] = await db.usuarios.find(
        {"tenant_id": tenant_id},
        {"_id": 0, "senha": 0}
    ).to_list(1000)
    
    tenant["empresas"] = await db.companies.find(
        {"tenant_id": tenant_id},
        {"_id": 0}
    ).to_list(1000)
...
async def update_tenant(tenant_id: str, tenant_data: TenantUpdate, current_user: Dict = Depends(require_super_admin)):
    ...
    await log_activity(
        ...
        detalhes=f"ID: {tenant_id}, Campos: {list(update_fields.keys())}"
    )
...
async def delete_tenant(tenant_id: str, current_user: Dict = Depends(require_super_admin)):
    """Exclui escritório e todos os dados relacionados"""
...
async def create_tenant_admin(tenant_id: str, user_data: UserCreate, current_user: Dict = Depends(require_super_admin)):
    ...
    await log_activity(
        current_user['id'], current_user['nome'],
        "Admin de escritório criado",
        detalhes=f"Usuário: {user.nome}, Escritório: {tenant['nome']}"
    )
...
@api_router.put("/superadmin/usuarios/{user_id}/reset-password")
...
@api_router.put("/superadmin/usuarios/{user_id}/toggle-status")
...
@api_router.post("/superadmin/init")
async def init_super_admin():
    """Inicializa o Super Admin da plataforma (executar apenas uma vez)"""
    ...
    super_admin = User(
        ...
        senha=hash_password("super123"),
        perfil=PERFIL_SUPER_ADMIN,
        tenant_id=None
    )
...
async def create_rule(rule: ClassificationRuleCreate, current_user: Dict = Depends(require_auth)):
    ...
    # Se especificou empresa, buscar o nome dela
    company_name = None
    if rule.company_id:
...
async def get_rules(company_id: Optional[str] = None, current_user: Dict = Depends(require_auth)):
    ...
    if company_id:
        # Retorna regras da empresa específica + regras globais (sem company_id)
        query["$or"] = [
            {"company_id": company_id},
            {"company_id": None},
